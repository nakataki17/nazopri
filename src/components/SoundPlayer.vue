  ...
  setup(props) {
        ...
        Object.keys(voiceData[course][imgInd]).forEach((time) => {
          ...
          if (audioList[fileName] === null) {
            const sound = new Howl({
              src: "https://nakataki17.github.io/nazopri_Sound/" + fileName,
            });
    ...
    watchEffect(() => {
      if (props.imageInd != 0) {
        if (
          props.time ==
            questionData[props.course][props.imageInd]["制限時間"] &&
          -1 in voiceData[props.course][props.imageInd]
        ) {
          const fileName =
            voiceData[props.course][props.imageInd][-1]["ファイル名"];
          setTimeout(() => {
            audioList[fileName].play();
          }, 500);
        } else {
          if (
            props.time in voiceData[props.course][props.imageInd] &&
            props.time > -1
          ) {
            const fileName =
              voiceData[props.course][props.imageInd][props.time]["ファイル名"];
